...
`summarize_auto_k_gates.py` now provides the schema-validated, deterministic
G1-G6 aggregation path. Once the missing raw timing file is recovered, the
required invocation shape is:
...
python benchmarks/summarize_auto_k_gates.py \
  ...
  --fixed-k-path-timing PATH/TO/RECORDED_D9_FIXED_K_SELECT_CACHED.csv \
  --oracle-aggregation mean \
  --output /tmp/auto_k_v2_gates.csv
```

The fourth input has schema
`design,seed,benchmark,runtime_s`, with `design=D9` and
`benchmark=fixed_k_select_cached`; its seeds must match the D9 method-timing
CSV. It is required because `auto_k_v2_d9.csv` records only the post-path
`fixed_k=50` dispatch time, not the full fixed-k `select_cached` denominator
specified by G5. That raw path-timing input was not committed with the 2026-07-08
campaign. The published gate CSV also does not declare whether the program
stability denominator used mean or median `k_oracle` (its original rows and
the later `gaussian_cv/best` row use different conventions). Therefore the
current `auto_k_v2_gates.csv` is intentionally left unchanged and cannot yet
be regenerated byte-identically from committed inputs. The summarizer refuses
to run until both missing provenance choices are supplied explicitly. Its
canonical output should then replace the mixed-convention legacy artifact only
after review.
